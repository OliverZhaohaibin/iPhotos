...
class RoundedWindowShell(QWidget):
    ...
    def resizeEvent(self, event: QResizeEvent) -> None:  # type: ignore[override]
        """Refresh the clipping mask whenever the shell changes size."""

        self._update_mask()
        super().resizeEvent(event)

    def _update_mask(self) -> None:
        """Limit child painting to the rounded outline drawn by the shell."""

        if self._corner_radius <= 0 or self.width() <= 0 or self.height() <= 0:
            # Clearing the mask restores the default rectangular clip whenever the window is
            # square-cornered or still zero-sized during initial construction.
            self.clearMask()
            return
        ...
        # ``QRegion`` accepts only integer polygons, so ``toFillPolygon`` converts the floating
        # point path into the closest pixel-aligned outline before applying it as a widget mask.
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, context: AppContext) -> None:
        ...
        # The autogenerated central widget needs to remain transparent so the rounded shell can
        # supply the only visible backdrop.  ``_apply_transparent_shell_palette`` centralises the
        # palette and attribute tweaks so we can reuse the logic when system themes change.
        self._apply_transparent_shell_palette()
        cast(QVBoxLayout, self._rounded_shell.layout()).addWidget(original_shell)
        self.setCentralWidget(self._rounded_shell)

        # The toolbar hosting the Rescan/Rebuild buttons and the custom status bar previously drew
        # their opaque fill from the parent palette.  Clearing the shell palette removes that link,
        # so we immediately reapply the intended colour to keep the chrome visually consistent.
    ...
    def _apply_transparent_shell_palette(self) -> None:
        """Keep the autogenerated central widget fully transparent."""
    ...
    def _refresh_opaque_surfaces(self) -> None:
        """Reapply the solid background colour to chrome rows."""

        surface_color = self._opaque_color(self.palette().color(QPalette.ColorRole.Base))
        for widget in (getattr(self.ui, "main_toolbar", None), getattr(self.ui, "status_bar", None)):
            if widget is None:
                continue
            self._apply_surface_palette(widget, surface_color)

    @staticmethod
    def _apply_surface_palette(widget: QWidget, color: QColor) -> None:
        """Set ``widget`` to paint an opaque ``color`` regardless of inherited palettes."""
    ...
    def changeEvent(self, event: QEvent) -> None:  # type: ignore[override]
        ...
        if event.type() == QEvent.Type.WindowTitleChange:
            ...
            # Refresh both the palette assignment and the cached painting so the rounded chrome and
            # its child widgets react instantly to theme updates without requiring a resize pass.
            self._rounded_shell.setPalette(self.palette())
