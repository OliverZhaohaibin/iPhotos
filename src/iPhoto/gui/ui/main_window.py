...
class MainWindow(QMainWindow):
    ...
    def __init__(self, context: AppContext) -> None:
        ...
        # ``WA_StyledBackground`` stays enabled so palettes continue to propagate through the
        # autogenerated widget hierarchy, but transparency is now driven by the palette rather
        # than a stylesheet.  This keeps the central shell invisible without introducing a
        # cascading style that could leak onto unrelated controls.
        original_shell.setAttribute(Qt.WidgetAttribute.WA_StyledBackground, True)
        original_shell.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground, True)
        original_shell.setAutoFillBackground(False)
        # Force the window roles in the palette to use a fully transparent colour.  Qt will skip
        # filling the background during paint events while child widgets remain free to draw their
        # own opaque surfaces when necessary.
        shell_palette = original_shell.palette()
        transparent = QColor(0, 0, 0, 0)
        shell_palette.setColor(QPalette.ColorRole.Window, transparent)
        shell_palette.setColor(QPalette.ColorRole.Base, transparent)
        original_shell.setPalette(shell_palette)
