...
class CustomScrollBarStyle(QProxyStyle):
    ...
    def _paint_groove(
        ...
        """Paint a rectangular track so only the handle receives rounded corners."""
        ...
        painter.setClipRect(option.rect)
        groove_rect = self._capsule_rect(option)
        # Draw a standard rectangle to keep the groove edges square while the handle
        # remains a capsule.  The inset generated by ``_capsule_rect`` still ensures
        # we leave a one-pixel padding around the geometry for crisp edges.
